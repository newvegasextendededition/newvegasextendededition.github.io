<script lang="ts">
  ...
  let donations: string | string[];
  let donateModal: HTMLDialogElement;
  export let donateModalOpenState: boolean;
  let incrementDonationIndexTimer: NodeJS.Timeout;
  let donationIndex: number = 0;

  // Fetch Donations
  async function fetchDonations() {
    const response = await fetch("https://raw.githubusercontent.com/newvegasextendededition/nveedocs/refs/heads/main/donators.txt");
    donations = await response.text();
    donations = donations.split("\r\n");
    return donations;
  }

  function incrementDonationIndex() {
    clearInterval(incrementDonationIndexTimer);
    incrementDonationIndexTimer = setInterval(() => {
      if (donations && donationIndex < donations.length - 1) {
        donationIndex += 1;
      } else {
        donationIndex = 0;
      }
  ...
  $: {
    if (donateModalOpenState) {
      donateModal.showModal();
      incrementDonationIndex();
    } else {
      clearInterval(incrementDonationIndexTimer);
      donationIndex = 0;
    }
...
<svelte:document on:visibilitychange={document.visibilityState === "hidden" || !donations ? () => clearInterval(incrementDonationIndexTimer) : () => incrementDonationIndex()} />

<!--* Donate Modal -->
<dialog
  on:close={() => (donateModalOpenState = false)}
  bind:this={donateModal}
  id="donate_modal"
  class="modal"
>
  <form
    method="dialog"
    class="modal-box relative flex max-w-xs flex-col pt-8 text-center sm:max-w-xs"
  >
    <!--* Title -->
    <h3 class="mb-4 text-xl uppercase lg:text-2xl">Реквизиты</h3>
    <!--*  Boosty Button -->
    <div class="mb-4 flex items-center justify-center gap-4 text-xl lg:text-2xl">
      <a
        href="https://boosty.to/root_"
        target="_blank"
        aria-label="Boosty"
        class="rounded-md bg-neutral p-3 transition-colors hover:bg-accent"
      >
        <!--* Boosty SVG -->
        <svg
    ...
    <div class="flex h-6 justify-center">
      {#if donateModalOpenState}
        {#await fetchDonations()}
          <div class="flex flex-1 items-center justify-center"><span class="loading loading-spinner loading-md"></span></div>
        {:then value}
          <div
            class="w-full"
            role="list"
            aria-roledescription="Список донатеров"
            on:mouseover={() => clearInterval(incrementDonationIndexTimer)}
            on:focus={() => clearInterval(incrementDonationIndexTimer)}
            on:mouseout={incrementDonationIndex}
            on:blur={incrementDonationIndex}
          >
            {#key donationIndex}
              <p
                class="text-base"
              ...
              >
                {@html value[donationIndex].replace("❤️", "")}
              </p>
            {/key}
          </div>
        {:catch error}
